<ul class="navbar-nav m-auto">
    ...
    @foreach ($menu_nodes as $key => $row)
        <li @if ($row->has_child || $row->css_class || $row->active) class=" @if ($row->has_child) dropdown @endif @if ($row->css_class) {{ $row->css_class }} nav-item  @endif @if ($row->active)  @endif" @endif>
            <a class="@if ($row->has_child) dropdown-toggle nav-link  nav-link  @else nav-link  @endif" href="{{ $row->has_child ? '#' : url($row->url) }}" @if ($row->target !== '_self') target="{{ $row->target }}" @endif @if ($row->has_child) data-toggle="dropdown" @endif>

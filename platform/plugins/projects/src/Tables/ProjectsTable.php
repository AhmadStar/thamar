...
class ProjectsTable extends TableAbstract
{
    ...
    public function ajax(): JsonResponse
    {
        $data = $this->table
            ->eloquent($this->query())
            ->editColumn('name', function (Projects $item) {
                if (! $this->hasPermission('projects.edit')) {
                    return BaseHelper::clean($item->name);
                }
                return Html::link(route('projects.edit', $item->getKey()), BaseHelper::clean($item->name));
            });
    ...
    {
        return $this->addCreateButton(route('projects.create'), 'projects.create');
    }
